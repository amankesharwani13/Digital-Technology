...
export const getProducts = catchAsyncErrors (async (req, res, next) => {
  const resPerPage = 4;  // 4 page show karne hai ek saath
  const apiFilters = new APIFilters(Product,req.query).search().filters();  // {{DOMAIN}}/api/v1/products?keyword=apple

  let products = await apiFilters.query; 
  let filteredProductsCount = products.length;  // count the filter product
